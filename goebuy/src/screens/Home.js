...
import { configureStatusBar, getSafeAreaStyle, getHeaderStyle } from "../utils/statusBar";
...
const Home = () => {
  ...
  const fetchCarrinhos = async (isRefresh = false, isFocusUpdate = false) => {
    try {
      ...
      // Log detalhado dos status dos carrinhos para debug
      if (data.length > 0) {
        const statusCount = data.reduce((acc, cart) => {
          const status = cart.isClosed ? 'fechado' : 
                        cart.isFinished ? 'finalizado' : 
                        cart.isCancelled ? 'cancelado' : 
                        !cart.isOpen ? 'não-aberto' : 'aberto';
          acc[status] = (acc[status] || 0) + 1;
          return acc;
        }, {});
        console.log(`[HOME] Distribuição de status:`, statusCount);
        
        // Log de carrinhos com data de fechamento já passada
        const hoje = new Date();
        const carrinhosVencidos = data.filter(c => c.closeDate && new Date(c.closeDate) <= hoje);
        if (carrinhosVencidos.length > 0) {
          console.log(`[HOME] ⚠️ ${carrinhosVencidos.length} carrinho(s) com data de fechamento já passada:`, 
            carrinhosVencidos.map(c => ({ nome: c.cartName, fechamento: c.closeDate })));
        }
      }
  ...
  const fetchUnreadNotifications = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filtrar mensagens de chat e contar apenas notificações não lidas relevantes
        const unreadCount = notificationsData.filter(notif => 
          !notif.read && !notif.isRead && notif.type !== 'message'
        ).length;
        console.log('[HOME-NOTIFICATIONS] Notificações não lidas (sem mensagens):', unreadCount);
  ...
  const onRefresh = async () => {
    ...
    // O setRefreshing(false) é feito dentro do fetchCarrinhos
  };
      ...
      try {
        ...
        socketRef.current.on('notification', (notif) => {
          ...
          if (notif.type === 'message') {
            console.log('[HOME-SOCKET] Ignorando notificação de mensagem');
            return;
          }
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      // Configurar StatusBar específico para Android
      ...
      // Atualizar dados da Home sempre que entrar na tela
      const updateHomeData = async () => {
        try {
          // Executar todas as funções em paralelo para maior eficiência
          await Promise.all([
            fetchCarrinhos(false, true), // false = não é refresh manual, true = é focus update
            fetchUserData(),
  ...
  const hoje = new Date();
  // Carrinhos prestes a fechar: fechamento nos próximos 3 dias e não fechados
  ...
  console.log("Carrinhos prestes a fechar:", carrinhosPrestesFechar);
  
  // Carrinhos recentes: abertura nos últimos 3 dias, não fechados e data de fechamento não passou
  const carrinhosRecentes = carrinhos.filter((c) => {
    if (!c.openDate) return false;
    
    // Verificar status do carrinho
    if (c.isClosed === true || c.isFinished === true || c.isCancelled === true || c.isOpen === false) {
      console.log(`[FILTRO RECENTES] Carrinho ${c.cartName} excluído - status inválido:`, {
        isClosed: c.isClosed,
    ...
    // Verificar se a data de fechamento ainda não passou
    if (c.closeDate) {
      const fechamento = new Date(c.closeDate);
      if (fechamento <= hoje) {
        console.log(`[FILTRO RECENTES] Carrinho ${c.cartName} excluído - data de fechamento já passou:`, fechamento.toLocaleDateString());
        return false;
      }
    }
    ...
    if (isRecent) {
      console.log(`[FILTRO RECENTES] Carrinho ${c.cartName} incluído - aberto há ${Math.floor((hoje - abertura) / (1000 * 60 * 60 * 24))} dias`);
    }
    
    return isRecent;
  });
  console.log("Carrinhos recentes:", carrinhosRecentes);
  ...
  const renderCarrinhoCard = ({ item }) => {
    ...
    // Determinar qual data mostrar e o texto
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {/* Container principal que força o layout correto */}
      <View style={styles.mainContainer}>
        ...
        <ScrollView 
          ...
          <RefreshControl 
            ...
            title="Atualizando carrinhos..." // iOS
