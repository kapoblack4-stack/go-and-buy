// ChatsScreen.js - Lista de conversas do vendedor
...
const ChatsScreen = ({ navigation }) => {
  ...
  const fetchConversations = async (isFocusUpdate = false) => {
    try {
      ...
      if (!token || !userIdStorage) {
        console.log('[CHATS-SCREEN] Token ou userId não encontrado');
        return;
      }

      setUserId(userIdStorage);
      console.log('[CHATS-SCREEN] Buscando conversas para vendedor:', userIdStorage);
      ...
      if (response.ok) {
        ...
        console.log('[CHATS-SCREEN] Conversas encontradas:', conversationsData.length);
        ...
        const conversationsWithDetails = await Promise.all(
          conversationsData.map(async (conversation) => {
            try {
              ...
              if (!lastMessage) {
                console.log('[CHATS-SCREEN] Conversa sem mensagens filtrada:', conversation._id);
                return null;
              }

              // Encontrar o outro membro da conversa (comprador)
              ...
              console.error('[CHATS-SCREEN] Erro ao processar conversa:', error);
              return null;
            }
          })
        );
        ...
        console.log('[CHATS-SCREEN] Conversas processadas:', validConversations.length);
        console.log('[CHATS-SCREEN] Conversas filtradas (apenas com mensagens):', 
        ...
        console.log('[CHATS-SCREEN] Erro ao buscar conversas:', response.status);
        Alert.alert('Erro', 'Não foi possível carregar as conversas');
      }
    } catch (error) {
      console.error('[CHATS-SCREEN] Erro geral:', error);
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('[CHATS-SCREEN-FOCUS] Tela de chats entrou em foco - forçando atualização completa');
      ...
      const timeoutId = setTimeout(() => {
        console.log('[CHATS-SCREEN-FOCUS] Timeout - fazendo segunda atualização');
        fetchConversations(true);
      }, 1000);
  ...
  const onRefresh = async () => {
    console.log('[CHATS-SCREEN] Pull-to-refresh');
    setRefreshing(true);
  ...
  useEffect(() => {
    const setupSocket = async () => {
      try {
        ...
        if (!userIdStorage) {
          console.log('[CHATS-SCREEN-SOCKET] UserId não encontrado');
          return;
        }

        console.log('[CHATS-SCREEN-SOCKET] Configurando socket para chats');
        ...
        socketRef.current.on('connect', () => {
          console.log('[CHATS-SCREEN-SOCKET] Socket conectado');
        });
        ...
        socketRef.current.on('receiveMessage', (message) => {
          console.log('[CHATS-SCREEN-SOCKET] Nova mensagem recebida:', message);
        ...
        socketRef.current.on('messagesMarkedAsRead', (data) => {
          console.log('[CHATS-SCREEN-SOCKET] Mensagens marcadas como lidas:', data);
        ...
        socketRef.current.on('message-read', (data) => {
          console.log('[CHATS-SCREEN-SOCKET] Evento message-read recebido:', data);
        ...
        socketRef.current.on('disconnect', () => {
          console.log('[CHATS-SCREEN-SOCKET] Socket desconectado');
        });

      } catch (error) {
        console.error('[CHATS-SCREEN-SOCKET] Erro ao configurar socket:', error);
      }
    };
    ...
    return () => {
      if (socketRef.current) {
        console.log('[CHATS-SCREEN-SOCKET] Desconectando socket...');
  ...
  const openChat = (conversation) => {
    console.log('[CHATS-SCREEN] Abrindo conversa:', conversation._id);
    
    navigation.navigate('ChatScreen', {
      conversationId: conversation._id,
      sellerId: userId,
      buyerId: conversation.otherUser._id,
      buyerName: conversation.otherUser.name
    });
  };
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>
              Suas conversas com mensagens trocadas aparecerão aqui
            </Text>
          </View>
        )}
      </View>
